...
export default class SearchableDropDown extends Component {
    ...
    searchedItems = searchedText => {
        ...
        if (!setSort && typeof setSort !== 'function') {
            setSort = (item, searchedText) => {
                return item[targetField].toLowerCase().indexOf(searchedText.toLowerCase()) > -1
            };
        }
                ...
                >
                    {
                        this.props.selectedItems && this.props.selectedItems.length > 0 && this.props.selectedItems.find(x => x.id === item.id)
                            ?
                            <Text style={{ ...this.props.itemTextStyle }}>{item[targetField]}</Text>
                            :
                            <Text style={{ ...this.props.itemTextStyle }}>{item[targetField]}</Text>
                    }
                </TouchableOpacity>
    ...
    renderTextInput = () => {
        ...
        const oldSupport = [
            ...
            {
                key: 'value',
                val: this.state.item ? this.state.item[targetField] : ''
            },
            {
                key: 'style',
                ...
                { items.map((item, index) => {
                    return (
                        <View key={index} style={{
                            width: (item[targetField].length * 8) + 60,
                            ...
                            <Text style={{ color: '#555' }}>{item[targetField]}</Text>
                            <TouchableOpacity onPress={() => setTimeout(() => { this.props.onRemoveItem(item, index) }, 0) } style={{ backgroundColor: '#f16d6b', alignItems: 'center', justifyContent: 'center', width: 25, height: 25, borderRadius: 100, marginLeft: 10}}>
                                <Text>X</Text>
                            </TouchableOpacity>
                        </View>
